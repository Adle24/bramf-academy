...
export default {
  ...
  methods: {
    ...
    selectAnswer(is_correct, test_type, phone, question_id, answer_id) {
      if (is_correct) {
        this.totalCorrect++
      }
      ...
      this.postQuestion(test_type, phone, question_id, answer_id)
        ...
        <main class="flex flex-col md:flex-row mb-auto h-full">
          ...
          <div class="bg-[#EEEEEE] p-8 md:p-16 md:w-1/2 flex flex-col gap-4">
            <div
              ...
              :key="answerInd"
              @click.prevent="
                selectAnswer(answer.is_correct, 1, this.phoneStore, question.id_q, answer.id_ans)
              "
            >
              <div>
                {{ answer.text }}
